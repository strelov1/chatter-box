class IoCContainer {
    ...
    registerMultiple(services) {
        services.forEach(({ token, useClass, useValue, useFactory, dependencies = [] }) => {
            if (useClass) {
                this.register(token, () => new useClass(...dependencies.map(dep => this.get(dep))), dependencies);
            } else if (useValue !== undefined) {
                this.registerValue(token, useValue);
            } else if (useFactory) {
                this.register(token, () => useFactory(...dependencies.map(dep => this.get(dep))), dependencies);
            }
        });
    }
